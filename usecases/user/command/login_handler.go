...
func (h *LoginHandler) Handle(command *LoginCommand) (*result.LoginInResult, error) {
	...
	// Verify password
	log.Println(user, "found this user","now checking password")
	ok, err := h.hashService.Match(user.Password(), command.password)
	if err != nil {
		return nil, err
	}
	if !ok {
	...
	// Return login result
	result := result.NewLoginInResult(token, user)
	return &result, nil
}
